...
import { novaIpeColors, novaIpeGradients } from '@/app/utils/nova-ipe-gradients';
...
function ImovelCard({
  ...
  const normalizedFinalidade = getFinalidade(imovel.finalidade || finalidade); const { images, priceLabel, address, title, disponivel, features, link, badgeColor, BadgeIcon, badgeLabel } = useMemo(() => {
    ...
    const hasDirectUrl = !!imovel.imagem?.imagemUrl;

    // Garante que só URLs válidas são adicionadas
    const mainImage = hasDirectUrl && typeof imovel.imagem?.imagemUrl === 'string' ? imovel.imagem.imagemUrl : null;

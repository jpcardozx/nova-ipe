...
export default function TasksPageProfessional() {
    ...
    const [viewMode, setViewMode] = useState<'board' | 'timeline' | 'priority' | 'calendar'>('board')
            ...
            <div className="max-w-7xl mx-auto px-6 py-8">
                ...
                <div className="flex flex-wrap items-center gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-2 bg-gray-100 rounded-lg p-1">
                        {[
                            { id: 'board', label: 'Quadro', icon: CheckCircle2 },
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-all ${viewMode === view.id
                ...
                {viewMode === 'calendar' ? (
                    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl font-bold text-gray-900">📅 Calendário de Tarefas</h2>
                        ...
                        <div className="grid grid-cols-7 gap-1 text-center text-sm">
                            ...
                            {generateCalendarDays(selectedDate).map((day, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className={`p-2 h-24 border border-gray-100 rounded relative ${day ? 'hover:bg-gray-50 cursor-pointer' : ''
                                                ...
                                                {tasksForDay.length > 0 && (
                                                    <div className="mt-1 space-y-1">
                                                        {tasksForDay.slice(0, 2).map((task, taskIndex) => (
                                                            <div
                                                                key={taskIndex}
                                                                className={`text-xs p-1 rounded truncate ${task.priority === 'urgent' ? 'bg-red-100 text-red-800' :
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                        <h2 className="text-xl font-bold text-gray-900 mb-6">📊 Timeline de Tarefas</h2>
                        <div className="space-y-4">
                            {filteredTasks.slice(0, 5).map((task, index) => (
                                <div key={task.id} className="flex items-start gap-4">
                                    <div className="flex flex-col items-center">
                                        <div className={`w-3 h-3 rounded-full ${task.priority === 'urgent' ? 'bg-red-500' :
                                            task.priority === 'high' ? 'bg-orange-500' :
                                                task.priority === 'medium' ? 'bg-yellow-500' : 'bg-green-500'
                                            }`}></div>
                                        {index < 4 && <div className="w-px h-8 bg-gray-200 mt-2"></div>}
                                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        {['urgent', 'high', 'medium', 'low'].map(priority => (
                            <div key={priority} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">

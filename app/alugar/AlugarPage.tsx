...
export default function AlugarPage() {
    ...
    const filtros = [
        ...
        { id: 'destaque', label: 'Em Destaque', icon: Star },
        { id: 'residencial', label: 'Residencial', icon: Home },
        { id: 'seguranca', label: 'Com Segurança', icon: Shield }
    ];

    const imoveisFiltrados = imoveis.filter(imovel => {
        const atendeFiltro = filtroAtivo === 'todos' ||
            (filtroAtivo === 'destaque' && imovel.destaque) ||
            (filtroAtivo === 'residencial' && imovel.dormitorios && imovel.dormitorios > 0) ||
            (filtroAtivo === 'seguranca' && imovel.endereco?.toLowerCase().includes('condomínio'));
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-slate-50">
            ...
            <section className="relative bg-gradient-to-br from-blue-900 via-slate-800 to-blue-900 text-white">
                ...
                <div className="relative max-w-7xl mx-auto px-6 py-24 pt-32">
                    <div className="max-w-4xl">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <span className="text-blue-300 font-semibold text-lg">Locação Especializada</span>
                        </div>

                        <h1 className="text-5xl sm:text-6xl font-bold leading-tight mb-6">
                            Imóveis para <span className="text-blue-400">Alugar</span>
                            <br />
                        ...
                        <p className="text-xl text-slate-200 leading-relaxed mb-8 max-w-3xl">
                            Imóveis cuidadosamente selecionados com <strong className="text-blue-300">boa localização</strong>,
                            segurança e <em>excelente custo-benefício</em> em Guararema e região.
                        </p>

                        <div className="flex flex-wrap gap-4">
                            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                                <Shield className="w-5 h-5 text-blue-400" />
                                <span className="text-sm font-medium">Segurança e Confiabilidade</span>
                            </div>
                            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                                <MapPin className="w-5 h-5 text-blue-400" />
                                <span className="text-sm font-medium">Localização Privilegiada</span>
                            </div>
                            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">

...
import { checkInstalledVersions, checkTailwindConfig, testTailwindClasses } from '../lib/tailwind-diagnostics';
...
export default function TailwindDiagnosticPanel() {
  ...
  useEffect(() => {
    async function runDiagnostics() {
      ...
      const tests = testTailwindClasses();
  ...
  const getOverallStatus = () => {
    ...
    const hasVersionIssues = diagnosticState.versions && (!diagnosticState.versions.success || 
      !diagnosticState.versions.versions.tailwind);
    
    const hasConfigIssues = diagnosticState.config && (!diagnosticState.config.success ||
      Object.values(diagnosticState.config.results).some(r => r.issues));
    
    const hasTestIssues = diagnosticState.tests && (diagnosticState.tests.environment === 'client' && 
      !diagnosticState.tests.working);
    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-200 flex items-center gap-2">
        <div className={`w-3 h-3 rounded-full ${
          overallStatus.status === 'loading' ? 'bg-blue-500 animate-pulse' :
          ...
          <>
            ...
            {diagnosticState.activeTab === 'versions' && (
              <div>
                ...
                {diagnosticState.versions.success ? (
                  <div className="space-y-3">
                    {Object.entries(diagnosticState.versions.versions).map(([pkg, info]) => (
                      <div key={pkg} className="bg-gray-50 p-3 rounded border border-gray-200">
                  ...
                  <div className="bg-red-50 text-red-700 p-3 rounded">
                    Erro ao verificar versões: {diagnosticState.versions.error}
                  </div>
                )}
              </div>
            )}
            ...
            {diagnosticState.activeTab === 'config' && (
              <div>
                ...
                {diagnosticState.config.success ? (
                  <div className="space-y-3">
                    {Object.entries(diagnosticState.config.results).map(([file, info]) => (
                      <div key={file} className={`p-3 rounded border ${
                        !info.exists || info.issues ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'
                      }`}>
                        <div className="flex justify-between items-center">
                          <span className="font-medium">{file}.js</span>
                          {info.exists ? (
                  ...
                  <div className="bg-red-50 text-red-700 p-3 rounded">
                    Erro ao verificar configuração: {diagnosticState.config.error}
                  </div>
                )}
              </div>
            )}
              ...
              <div>
                ...
                {diagnosticState.tests.environment === 'server' ? (
                  <div className="bg-yellow-50 text-yellow-700 p-3 rounded">
                    {diagnosticState.tests.message}
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className={`p-3 rounded ${
                      diagnosticState.tests.working ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
                    }`}>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {diagnosticState.tests.results.map(result => (
                        <div
                          key={result.class}
                          className={`p-2 text-sm rounded flex items-center ${
                            result.applied ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-200'
                          }`}
                        >

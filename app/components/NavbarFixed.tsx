...
export default function NavbarFixed() {
    ...
    // Add debugging log
    console.log("NavbarFixed rendering, isMounted:", isMounted);
    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">                {/* Logo */}
                ...
                <ul className={`flex gap-4 md:gap-10 text-[#0D1F2D] text-sm font-medium ${montSerrat.className}`}>
                    {links.map(({ label, href }) => (
                        <li key={label}>
                            <Link
                                href={href}
                                className={`relative group transition-colors ${isMounted && pathname === href ? "text-[#FFAD43]" : "hover:text-black"}`}
                            >
                                {label}
                ...
                <motion.a
                    ...
                    className="inline-flex items-center gap-2 bg-[#20b858] text-white px-3 py-1 md:px-5 md:py-2 rounded-full text-sm font-medium hover:brightness-105 transition shadow-sm"
                >
                ...
                </motion.a>                {/* Menu Toggle Mobile - completely hidden for now */}
                <button
                    onClick={() => setOpen(!open)}
                    className="hidden text-gray-700 text-2xl items-center justify-center"

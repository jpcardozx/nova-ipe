...
const FEATURE_ENABLED_ENV_NAME = "SANITY_CLI_SCHEMA_STORE_ENABLED", SCHEMA_STORE_FEATURE_ENABLED = process.env[FEATURE_ENABLED_ENV_NAME] === "true", defaultApiVersion$1 = "v2024-02-21", datasetBackupGroup = {
  name: "backup",
  signature: "[COMMAND]",
...
const debug = debug__default.default("sanity:backup"), DEFAULT_DOWNLOAD_CONCURRENCY = 10, MAX_DOWNLOAD_CONCURRENCY = 24, helpText$E = `
Options
...
}, DEFAULT_LIST_BACKUP_LIMIT = 30, helpText$C = `
Options
...
const yellow = (str) => `\x1B[33m${str}\x1B[39m`, helpText$s = `
Options
...
const schemaStoreText = ["  --schema-required Fail-fast deployment if schema store fails", "  --verbose Enable verbose logging", ""].join(`
`), examples = `Examples
  # Build the studio
  sanity deploy

  # Deploys non-minified build with source maps
  sanity deploy --no-minify --source-maps`, examplesWithSchemaRequired = `${examples}

  # Fail fast on schema store fails \u2013 for when other services rely on the stored schema
  sanity deploy --schema-required`, helpText$r = `
Options
  --source-maps Enable source maps for built bundles (increases size of bundle)
  --auto-updates / --no-auto-updates Enable/disable auto updates of studio versions
  --no-minify Skip minifying built JavaScript (speeds up build, increases size of bundle)
...
${SCHEMA_STORE_FEATURE_ENABLED ? examplesWithSchemaRequired : examples}`, deployCommand = {
  name: "deploy",
...
}, defaultApiVersion = "v2022-06-01", helpText$l = `
...
const description$6 = "Downloads and validates all document specified in a workspace", helpText$k = `
Options
...
}, description$5 = "Extracts the studio configuration as one or more JSON manifest files.", helpText$f = `
